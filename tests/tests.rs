...
fn missinig_tags() {
    ...
    FixMessage::from_tag_value(&input).unwrap();
}

#[test]
#[should_panic]
fn value_with_separator() {
    // you can't use a sepataor in a value, unless it is a binary field (nice-to-have)
...
#[should_panic]
fn too_short() {
    let input = "8=FIX.4.4|10=20"; // checksum should at least have 3 characters
    FixMessage::from_tag_value(&input).unwrap().to_json().to_string();
}

#[test]
#[should_panic]
fn missing_fix_version() {
    let input = "8= | 9=some | 10=123";
    FixMessage::from_tag_value(&input).unwrap().to_json().to_string();
}
